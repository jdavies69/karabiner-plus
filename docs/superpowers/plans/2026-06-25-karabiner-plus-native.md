# Karabiner+ Native Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a native Swift macOS app shell named Karabiner+ that covers setup, opt-in active-app usage tracking, recommendations, and safe global shortcut creation for Karabiner-Elements.

**Architecture:** Add a Swift package-style source tree with testable `KarabinerPlusCore` logic and a SwiftUI/AppKit `KarabinerPlusApp`. Keep the existing Node prototype intact while shifting README/docs and build output to the native app.

**Tech Stack:** Swift 6.3 command-line tools, SwiftUI, AppKit, a plain Swift `KarabinerPlusCoreCheck` executable for local verification, existing Node tests for the legacy prototype.

## Global Constraints

- Native app name is Karabiner+.
- The app is a windowed macOS app with Dock icon; no menu bar status item.
- macOS only.
- Tracking runs only while Karabiner+ is open and after the user starts it.
...
- Create: `Sources/KarabinerPlusCore/ShortcutDefinition.swift`
- Create: `Sources/KarabinerPlusCore/UsageTracker.swift`
- Create: `Sources/KarabinerPlusCore/RecommendationEngine.swift`
- Create: `Sources/KarabinerPlusCoreCheck/main.swift`
...
- [ ] Run `swift run KarabinerPlusCoreCheck` and confirm missing type/function failures.
- [ ] Implement the minimal Swift core models and engines.
- [ ] Run `swift run KarabinerPlusCoreCheck` and confirm it passes.
- [ ] Commit this task.

### Task 2: Karabiner Config Service

**Files:**
- Create: `Sources/KarabinerPlusCore/KarabinerConfigService.swift`
- Modify: `Sources/KarabinerPlusCoreCheck/main.swift`
...
- [ ] Run `swift run KarabinerPlusCoreCheck` and confirm the new check fails due to missing service behavior.
- [ ] Implement file read/write, backup creation, category-aware starter rule merging, and basic conflict detection.
- [ ] Run `swift run KarabinerPlusCoreCheck`.
- [ ] Commit this task.

### Task 3: Native SwiftUI App

**Files:**
- Create: `Sources/KarabinerPlusApp/KarabinerPlusApp.swift`
- Create: `Sources/KarabinerPlusApp/AppModel.swift`
- Create: `Sources/KarabinerPlusApp/ContentView.swift`
- Create: `Sources/KarabinerPlusApp/SetupView.swift`
- Create: `Sources/KarabinerPlusApp/CoachView.swift`
- Create: `Sources/KarabinerPlusApp/StudioView.swift`
- Create: `Sources/KarabinerPlusApp/SafetyView.swift`

**Interfaces:**
- Consumes: `KarabinerPlusCore`
...
**Files:**
- Create: `build.sh`
- Create: `Sources/KarabinerPlusApp/Info.plist`
- Modify: `.gitignore`

**Interfaces:**
- Produces: `build/Karabiner+.app`
...
- [ ] Run `./build.sh`.
- [ ] Run `open build/Karabiner+.app` for a smoke launch if possible.
- [ ] Commit this task.
...
**Interfaces:**
- Documents Karabiner+ as the primary app and the Node browser UI as a legacy prototype/fallback.
